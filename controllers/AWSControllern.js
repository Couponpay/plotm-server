                        ...
                        Jimp.read(nfile_Original, (err, image) => {
                            image.resize(item.width, Jimp.AUTO)
                                ...
                                .write(item.nfile, (err, Result) => {
                                    ...
                                    AWSController.Upload_FILE_AWS(item.nfile, item.fname, contentType).then((UploadStatus) => {
                                        callback();
                                    }).catch((err) => console.log(err));
                                })
                        });
                    }, (err) => {
                        AWSController.Upload_FILE_AWS(nfile_Original, ImageOriginal, contentType).then((UploadStatus) => {
                            console.log("Image Functionality Completed");
                        }).catch((err) => console.log(err));
                    });
                })
            } else {
    ...
    newForm.on('end', () => {
        if (contentSize <= config.File_Size) {
            if (filesplit.length > 1) {
                ...
                fs.rename(tmpFile, nfile_Original, () => {
                    AWSController.Upload_FILE_AWS(nfile_Original, Document_URL, contentType).then((UploadStatus) => {
                        if (UploadStatus == "AWS Upload Fails") {
                            res.json({ success: false, extras: { Status: "Update Failed" } });
                        } else {
                            console.log("url", config.S3URL + Document_URL);
                            res.json({
                                success: true,
                                extras: {
                        ...
                        console.log("File Functionality Completed");
